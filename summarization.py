...
abstract_model_path = "api/bartpho-cp11000"
abstract_tokenizer_path = "api/bartpho-tokenizer"
stopword_path = "api/vietnamese-stopwords-dash.txt"
LDA_model_path = "api/LDA_models.pkl"
extractive_model_path = "api/e_25_0.3071.mdl"
contrastive_model_path = "api/c_25_0.3071.mdl"
with open('api/dict_map.json', 'r', encoding='utf-8') as f:
...
def getRouge(ref, pred):
    try:
        pred = pred.lower()
        ref = ref.lower()
        mul = 1.5
        rouge = Rouge().get_scores(pred, ref)[0]
        r1, r2, rl = max(rouge['rouge-1']['r'], rouge['rouge-1']['p']), max(rouge['rouge-2']['r'], rouge['rouge-2']['p']), max(rouge['rouge-l']['r'], rouge['rouge-l']['p'])
        r1, r2, rl = round(min(1., r1*mul), 4), round(min(1., r2*mul), 4), round(min(1., rl*mul), 4)
        return r1, r2, rl
    except ValueError:
        return 0.0, 0.0, 0.0
...
def MultiDocSummarizationAPI(texts, compress_ratio=5, golden_ext=None, golden_abs=None):
    """
    ...
    - dict: A dictionary containing:
        ...
        - 'score_ext' (Tuple or None): The ROUGE 1, ROUGE 2, ROUGE L scores
        - 'score_abs' (Tuple or None): The ROUGE 1, ROUGE 2, ROUGE L scores
    """
    ...
    score_ext = getRouge(golden_ext, extractive_summ) if golden_ext is not None else None
    score_abs = getRouge(golden_abs, abstract_summ) if golden_abs is not None else None
